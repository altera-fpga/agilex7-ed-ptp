...
# There are no simulations runs for ANLT DESIGNS.
...
1) Initial set all tool paths vcs, python etc as follow. 
      ...
      VCS          : vcsmx/V-2023.12-SP2-1
      Python       : python/3.7.7
      Quartus      : Quartus Version 25.1.1
      PERL         : perl/5.8.8
      CMAKE        : cmake/3.11.4
      GCC          : gcc/7.2.0
      PTP_ROOTDIR : <user path>/<repo name>/src/hw/verification
...
2)  Set all the environemnt variables as per the setup.sh
      Refer <user path>/<repo name>/src/hw/verification/env/setup.sh
      WORKDIR=$PTP_ROOTDIR
...
verification
    ...
    │   │   ├── fptp_usr_base_test.svh                -> This test is to check the packet client blocks only. Packet client0 will send data to HSSI Port1 an gets looped back to Packet client0. Packet client1 will send data to HSSI Port2 an gets looped back to Packet client1.
    │   │   ├── fptp_usr_lpbk_test.svh                -> This test is to check the packet client blocks only with port cross level loopback ie,  Packet client0 will send data to HSSI Port1. HSSI Port1 will loopback to HSSI Port2. Port2 will send to Packet client1. Similarly Packet client1 data gets cross looped back and reaches Packet client0.
    ...
    │       ├── fptp_usr_base_seq.sv                -> QOS Usr Sequence for QOS USR test (fptp_usr_base_test).
    │       ├── fptp_usr_lpbk_seq.sv                -> QOS Usr Sequence for QOS USR test (fptp_usr_lpbk_test).
...
As mentioned above there are 4 variants for NON-ANLT UVM DV Simulation

2)  
    Design consists of various IPs. When below command is executed, all IPs will be generated and its relevant libraries will also be generated and compiled. Additionally the testbench uses AXI VIP for initiating transactions and vip folder also will also be generated. This command has to be generated only once for the entire simulation.

    #10G NON-ANLT Design
    For Compiling IPs and Subsystems, execute: “gmake -f Makefile.mk cmplib  HSSI_10G=1 | tee cmp.log” 

    #25G NON-ANLT Design
    For Compiling IPs and Subsystems, execute: “gmake -f Makefile.mk cmplib  HSSI_25G=1 | tee cmp.log” 

    #50G NON-ANLT Design
    For Compiling IPs and Subsystems, execute: “gmake -f Makefile.mk cmplib  HSSI_50G=1 | tee cmp.log” 

    #100G NON-ANLT Design
    For Compiling IPs and Subsystems, execute: “gmake -f Makefile.mk cmplib  HSSI_100G=1 | tee cmp.log” 
...
3)  
    ...
    #10G NON-ANLT Design
    For building RTL & TB, execute: “gmake -f Makefile.mk build  HSSI_10G=1 [DUMP=1]” [ DUMP is an option. If VPD is required, then use DUMP else it is not required]

    #25G NON-ANLT Design
    For building RTL & TB, execute: “gmake -f Makefile.mk build  HSSI_25G=1 [DUMP=1]” [ DUMP is an option. If VPD is required, then use DUMP else it is not required]

    #50G NON-ANLT Design
    For building RTL & TB, execute: “gmake -f Makefile.mk build  HSSI_50G=1 [DUMP=1]” [ DUMP is an option. If VPD is required, then use DUMP else it is not required]

    #100G NON-ANLT Design
    For building RTL & TB, execute: “gmake -f Makefile.mk build  HSSI_100G=1 [DUMP=1]” [ DUMP is an option. If VPD is required, then use DUMP else it is not required]
    
Note: 
Build command will take 30-40mts to execute.

4)  For tests run execute: “gmake -f Makefile.mk run TESTNAME=<TESTNAME> SEQNAME=<SEQNAME> <SEED=<seed number>>" [SEED is an option.To rerun the failure test, SEED=<seed number> is used]
    E.g. To run fptp_csr_test the command is 
    “gmake -f Makefile.mk run TESTNAME=fptp_csr_test SEQNAME=fptp_csr_seq" 
   ...
   To run with a failing seed 
   E.g.“gmake -f Makefile.mk run TESTNAME=fptp_csr_test SEQNAME=fptp_csr_seq SEED=12345678" (For seed number refer the simulate_$TESTNAME.log in sim dir)

Note:
Test run command will take 1-2 hrs of time. Once the test run starts, it will wait for the HSSI link up for both the ports to be ready. Once link is up, test flow starts. For all the tests, initial configuration needs to be done and then data traffic starts. 
...
3.fptp_qos_usr_test.svh
  fptp_qos_usr_seq.sv  -> Extended from base seq.Has instances of fptp_data_traffic_cfg_seq,fptp_ptp_bridge_cfg_usr_seq, fptp_ptp_bridge_cfg_dma_seq which does initial configuration,fptp_axi_slave_host_response_seq and fptp_user_traffic_check_seq

4.fptp_usr_base_test.svh
  fptp_usr_base_seq.sv  -> Extended from base seq.Has instances of fptp_data_traffic_cfg_seq,fptp_ptp_bridge_cfg_usr_seq, fptp_ptp_bridge_cfg_dma_seq which does initial configuration,fptp_axi_slave_host_response_seq and fptp_user_traffic_check_seq

5.fptp_usr_lpbk_test.svh
  fptp_usr_lpbk_seq.sv  -> Extended from base seq.Has instances of fptp_data_traffic_cfg_seq,fptp_ptp_bridge_cfg_usr_seq, fptp_ptp_bridge_cfg_dma_seq which does initial configuration,fptp_axi_slave_host_response_seq and fptp_user_traffic_check_seq
