...
input [3:0] status_curr_fsm_state_i,
input read_timeout_fsm_flag,
input clk,
input reset,
...
input [5:0] address,
output read_timeout_fsm_flag_pe
);

wire reset_n = !reset;	
reg [63:0] rdata_comb;
reg [63:0] scratch_reg;
reg poller_fsm_to_reg;
//reg poller_fsm_to_reg_1;
reg read_timeout_fsm_flag_dly;
...
wire wr_clear_pft = wr & (addr[5:0]== 6'h28)?	byteenable[0]:1'b0; // poller fsm timeout
wire wr_scratch_reg = wr & (addr[5:0]  == 6'h30)? byteenable[0]:1'b0;
wire wr_delay_fsm_reg = wr & (addr[5:0]  == 6'h38)? byteenable[0]:1'b0;

assign read_timeout_fsm_flag_pe = read_timeout_fsm_flag & ~read_timeout_fsm_flag_dly;
...
always @( negedge  reset_n,  posedge clk)
   ...
   if (!reset_n)  begin
     poller_fsm_to_reg  <= 1'h0;
     read_timeout_fsm_flag_dly <= 1'h0;
   end 
   else begin
      read_timeout_fsm_flag_dly <= read_timeout_fsm_flag;

      if (read_timeout_fsm_flag_pe) begin
        poller_fsm_to_reg <= 1'b1;
      end
      else if (wr_clear_pft) begin 
      poller_fsm_to_reg   <=  din[28];       
   end  
end
...
rdata_comb = 64'h0000000000000000;
   if(re) begin
	...
	6'h28 : begin
		...
		rdata_comb [27:24]  = status_curr_fsm_state_i;          //
		rdata_comb [28]	    = poller_fsm_to_reg;                                 // timeout status
		rdata_comb [63:29]	= 40'h0000000000;               // status_rsvd3 	is reserved or a constant value, a read access gives the reset value
	end
	6'h30 : begin
