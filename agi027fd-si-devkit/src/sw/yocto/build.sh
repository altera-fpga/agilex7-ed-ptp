...
if [[ "$IMAGE" != "gsrd" ]]; then
	if [[ "$MACHINE" == *"dk_si_agi027fb"* || "$MACHINE" == *"dk_si_agi027fc"* ]]; then
		if [[ "$IMAGE" == *"PTP_2P10G_MCQ_ANLT"* ]]; then
			...
			SOLUTION=${solution}
		elif [[ "$IMAGE" == *"PTP_2P25G_MCQ_ANLT"* ]]; then
			solution="PTP_2P25G_MCQ_ANLT"
			SOLUTION=${solution}
		elif [[ "$IMAGE" == *"PTP_2P50G_MCQ_ANLT"* ]]; then
			solution="PTP_2P50G_MCQ_ANLT"
			SOLUTION=${solution}
		elif [[ "$IMAGE" == *"PTP_2P100G_MCQ_ANLT"* ]]; then
			solution="PTP_2P100G_MCQ_ANLT"
			SOLUTION=${solution}
		elif [[ "$IMAGE" == *"PTP_2P10G_MCQ"* ]]; then
			solution="PTP_2P10G_MCQ"
			SOLUTION=${solution}
		elif [[ "$IMAGE" == *"PTP_2P25G_MCQ"* ]]; then
			solution="PTP_2P25G_MCQ"
			SOLUTION=${solution}
		elif [[ "$IMAGE" == *"PTP_2P50G_MCQ"* ]]; then
			solution="PTP_2P50G_MCQ"
			SOLUTION=${solution}
...
if [ -n "${PTP_SW_VERSION_STRING}" ]; then
	...
	SW_VERSION_STRING=${ptp_sed_sw_version}
fi
...
#------------------------------------------------------------------------------------------#
	pushd $WORKSPACE > /dev/null
		...
		if [[ "$MACHINE" == "agilex7"* && -n "${SOLUTION}" ]]; then
			bitbake-layers add-layer ../meta-agilex7-sed
		fi
