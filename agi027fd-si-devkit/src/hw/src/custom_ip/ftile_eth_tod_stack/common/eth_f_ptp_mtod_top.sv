...
#(
    parameter MASTER_PPS_CYCLE = 100000
)(
    // Clock and Reset
    ...
    //---------------------------------------------------------------
    // Scan clock and reset
    assign pps_pll_scanclk = i_clk_reconfig; // any clock 50-100 MHz
    assign pps_pll_rst_n = i_reconfig_rst_n; // reset in scan clock domain
	
    // -----------------------------------------------
    // Single PLL for sampling clock and phased clock
    ghrd_f_mtod_pps_pll ghrd_mtod_pps_pll (
        .refclk     (i_clk_tod),
        .rst        (~pps_pll_rst_n),
        .outclk_0   (pps_samp_clk),     // ~256/375 of period clock, e.g. 106.67 (actual 108.17 OK)
        .outclk_1   (pps_phased_clk),   // same as period clock, e.g. 156.25
    ...
    // -----------------------------------------------
    // wiring between reconfig IP and TOD
    ...
    // Master TOD
    ghrd_f_master_tod #(
        .PPS_PULSE_ASSERT_CYCLE_MASTER  (MASTER_PPS_CYCLE)	// Propagate PPS pulse width parameter
    ) ghrd_mtod (
        .clk                         (i_clk_reconfig),
        ...
        .iopll_phased_clk       (pps_phased_clk),       // phased period_clk
        .iopll_scan_clk         (pps_pll_scanclk),      // any clock 50-100 MHz
