...
module poller_fsm (
  ...
  output  logic [3:0]  curr_fsm_state,
  output  logic        read_timeout_fsm_flag
);
  ...
  logic  [31:0] read_timeout;
  logic  [31:0] read_timeout_nxt;
  logic         read_timeout_flag;
  logic         read_timeout_flag_nxt;
  ...
  assign curr_fsm_state = state;
  assign read_timeout_fsm_flag = read_timeout_flag; 
...
always_comb
  begin
     ...
     read_timeout_nxt = read_timeout;
     read_timeout_flag_nxt = read_timeout_flag;

  case(state)
    IDLE: 
    begin
      delay_st_sp_nxt = 32'd0;
      read_timeout_nxt = 32'h00FF_FFFF;
      read_timeout_flag_nxt = 1'b0;
      if(poll_en)
	   begin
        state_nxt = WRCW;
	   end
    ...
    begin
      if(read_timeout == 32'h0000_0000)
      begin
        read_timeout_flag_nxt = 1'b1;
        wren_logic_nxt  = 0;
        state_nxt = IDLE;
      end
      read_timeout_nxt = read_timeout - 1;
    end
  end
  ...
  begin
    ...
    begin
      ...
      wr_cnt_rst  <= 1'b0;
	  delay_st_sp <= 32'd0;
      read_timeout <= 32'h00FF_FFFF;
      read_timeout_flag <= 1'b0;
    end
    ...
    begin
      ...
      read_timeout <= read_timeout_nxt;
      read_timeout_flag <= read_timeout_flag_nxt;
    end
  end
